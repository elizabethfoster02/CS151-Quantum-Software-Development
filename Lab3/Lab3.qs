...
namespace Lab3 {
    ...
    operation Exercise2 (register : Qubit[]) : Unit {
        let i = Length(register);
        let var1 = (i) / 2;
        for j in 0 .. var1 - 1 {
            SWAP(register[j], register[i - j - 1]);
        }
    }
    ...
    operation Exercise3 (registers : Qubit[][]) : Unit {
        ...
        H(registers[2][0]);
        CNOT(registers[2][0], registers[2][1]);
        X(registers[2][1]);

        H(registers[3][0]);
        CNOT(registers[3][0], registers[3][1]);
        X(registers[3][1]);
        Z(registers[3][0]);
    }
    ...
    operation Exercise4 (register : Qubit[]) : Unit {
        let i = Length(register);
        for j in 1 .. i - 1 {
            Controlled Z(register[0 .. j - 1], register[j]);
        }
    }
    ...
    operation Exercise8 (register : Qubit[]) : Unit {
        ...
        X(register[2]);
        Controlled Z([register[2]], register[0]);
        X(register[2]);
    }
